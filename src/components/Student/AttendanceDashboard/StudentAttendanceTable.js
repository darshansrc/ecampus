...
function StudentAttendanceTable({  }) {
  ...
  useEffect(() => {
    if (chartRef && chartRef.current) {
      const attendancePercentages = attendanceData.map(
        (data, index) => getAttendancePercentage(index)
      );
  ...
  return (
    ...
    <p>Lectures: {totalClassesAttended}/{totalClassesHeld}</p>
      <table className='mtable' style={{marginTop: '20px'}}>
        <thead>
          <tr>
            <th>Subject Code</th>
            <th>Classes Held</th>
